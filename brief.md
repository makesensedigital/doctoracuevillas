...
| Photography | Five phone photographs of the practitioner, supplied 2026-08-09. Three are used, one of them on two pages. **Publication is authorised** — she sent them for this, confirmed through Juan. They are not a professional session and **no session is pending**: one was suggested, the suggestion was passed on, and that is where it rests. | Juan | 2026-08-12 |
| Where she is seen in person | **Two rooms, and only two.** The Austral's **consultorios externos at the Sede Escobar**, and Centro Médico Villanueva. She is médica de staff of the Austral's Servicio de Endocrinología — an affiliation to a hospital whose campus is in Pilar — and `/sobre-mi` carries that as an affiliation, never as an address. **Pilar was published as a consulting room until 2026-08-12 and is not one.** An address on this site is an invitation to travel to it. | Guadalupe, through Juan | 2026-08-12 |
| The length of the climaterio consultation | **Not published — a decision, not a gap.** Fertilidad and endocrinología state a duration; climaterio does not and will not. The scheduler's 45-minute default is the provider's setting and is not evidence of anything. **Do not raise this again.** | Juan | 2026-08-12 |
| What the site says about Villanueva | The address, that attention is by agenda, and that OSDE turnos are taken through cartilla or the app. **Nothing further about how she attends there** — no private presencial offer, no modality, no hours. Asked and closed. | Juan | 2026-08-12 |
| "Especializada", not "especialista", outside her one titled specialty | In Argentina *especialista* names a titled specialty with its own registration. Hers is **Endocrinología**, and `/sobre-mi` says *Especialista en Endocrinología* because that is exactly what it is — **the only place the word survives on this site.** Climaterio is international training and fertilidad is Medical Consultant formation in NaProTecnología, so both read **especializada**: the credential chips on the home page, `/climaterio` and `/endocrinologia`, the `<h2>` on `/fertilidad`, and two meta descriptions. | Guadalupe, through Juan | 2026-08-12 |
...
| 4 | Retired URLs and the redirect plan | The predecessor is `sites.google.com/view/naprofertility`, on a domain this project does not control. **No redirect can be served from here**, and GitHub Pages cannot serve rule-based redirects at all — `_redirects` is inert and says so at the top. The one redirect this site needs, `www` to the apex, is done by the host from the `CNAME` file. The equity on the old URLs is recovered by editing that page down to one line pointing here. **CHECKED 2026-09-21 and NOT DONE: it answers 200 with its full content and its booking buttons point at the same two schedulers this site uses, so it is a second live funnel rather than a relic — see the pending item below.** The retired scheduler `naprofertilitydracuevillas.youcanbook.me` is de-indexed at the provider and, as of 2026-09-21, answers 404; never link it. | Juan |
...
|---|---|---|---|
| ~~**Three steps, in order: (1) Settings → Pages → Source: GitHub Actions. (2) DNS for the apex and `www` pointed at GitHub Pages, then "Enforce HTTPS". (3) Repository variable `PUBLISH_ENABLED=true`.**~~ **ALL THREE DONE — verified 2026-08-11.** | Nothing. The site is published, and merging to `main` now deploys | Juan | Pages source is `workflow`, custom domain `doctoracuevillas.com`, HTTPS enforced on a certificate covering the apex and `www`. `PUBLISH_ENABLED` was set `true` on 2026-08-11. The apex answers 200 and `www` redirects to it. `CNAME` travels with the artifact and the job refuses to deploy without it. **What this changes for everyone working here: a merge is a publication.** |
...
| ~~**Scheduling link for climaterio**~~ **SUPPLIED 2026-08-11 — closed.** | Nothing. `/climaterio` is no longer the one door that cannot send anyone to a scheduler | Juan | `climaterio.youcanbook.me`, supplied by Juan and **opened**: HTTP 200, status `ONLINE`, titled "Consultorio CLIMATERIO / TRANSICION MENOPAUSICA", Buenos Aires timezone. Stored in `config.agendas.climaterio` without the `utm_*`/`fbclid` string it was shared under — see section 5. The four controls on the page moved to the scheduler **with their labels** in the same commit (hero, cierre, tarjeta, sticky). `/como-es-la-consulta` gained its own climaterio button, and the WhatsApp button beside it went from "Climaterio y otras consultas" to "Otras consultas". |
| ~~**The length of the climaterio consultation**~~ **NOT A PENDING ITEM — 2026-08-12. It is a decision not to publish it**, and it moved to section 1. | Nothing | — | Asked on 2026-08-12 and answered: it stays unpublished. The row is struck rather than deleted so that the next pass reads "decided" instead of rediscovering the gap and asking again. |
| ~~Guadalupe's decision on publishing the private presencial consultation at Villanueva~~ **CLOSED 2026-08-12 — nothing further is published.** | Nothing | — | Answered: no additional information about how she attends at Villanueva. `/endocrinologia` keeps saying only that she sees patients there "por los canales de cada institución", and the card on `/contacto` keeps the address, the agenda line and the OSDE route. In section 1. |
| ~~**The street number of the Escobar consulting room**~~ **SETTLED 2026-08-11 at Juan P. Asborno 650 — after shipping as 642 earlier the same day.** | Nothing | Guadalupe | Sequence, because a fact that moved twice in a day can move again: 642 was the standing instruction and Guadalupe confirmed it; hours later Juan instructed 650, which is also what the hospital's own page for this sede (`/pacientes/sedes/escobar/`) publishes — so the site and the first source anybody checks now agree, and that is what stops this being re-litigated. 642 survives in the hospital's 2014 opening note, in its Plan de Salud sucursal listing and in the scraped directories, so **a source saying 642 is not new information**. Before moving it a third time, get it from somebody who has been to the door recently and name them in the commit. The number is declared once, in `facts.js`; the card on `/contacto`, the footer, the structured data, `llms.txt` and the map link derive from it. |
| ~~**Whether she attends in Pilar at all**~~ **SETTLED 2026-08-12, same day it was removed: SHE DOES NOT.** | Nothing | — | She is médica de staff of the Austral's Servicio de Endocrinología and sees patients in person **only in that hospital's consultorios externos at the Sede Escobar**. The Pilar entry was in the first build, taken from the brief, and was never separately confirmed; the Escobar sede was added **beside** it on 2026-08-11 rather than in its place, which is what put both on the page at once. **The 642 → 650 correction did not establish this and was never evidence about Pilar** — it was the Escobar street number. Recorded so the affiliation is not mistaken for an address a second time: `/sobre-mi` names the Austral because she is staff there, and that is the only place Pilar's hospital appears. **What it cost:** `Pilar` left the `<title>` of `/endocrinologia` with it, so a search that used to reach this site no longer does. That is the price of not inviting a patient to a door she cannot be seen at. |
| ~~The hero photograph for `/sobre-mi` — **a stopgap is in place since 2026-08-11**~~ **ACCEPTED AS IT STANDS, 2026-08-12. The duplication stays and nobody is waiting on a file.** | Nothing. It costs a **duplicated photograph**: the page shows the same image as `/climaterio`, framed wider — now a decision rather than a stopgap | — | The five photographs supplied on 2026-08-09 are all in use — `11.22.10` → climaterio *and ahora sobre mí*, `11.22.29` → endocrinología y la consulta, `11.22.36` → fertilidad, `11.22.45` → el retrato del home, `11.22.23` → el retrato de pasillo que estaba acá. Four are 960×1280 and publish at 760×950; **the corridor one is 640×640**, so its 4:5 crop is 512×640 and the browser was stretching it into the 760×950 box. That was the softness — not the encoding, which at 49 KB/MP sat inside the range of the others. There was no sixth photograph on disk. Sharpness was chosen over variety. **What would end the duplication, if anyone ever wants to:** a full-size original of the corridor portrait (posed, Mater Dei lanyard — whoever took it has it) or a photograph taken for this page. Neither is being chased. Overwriting `assets/guada-sobremi.webp` then requires bumping `config.assetVersion`, because the URL does not change and `?v=` is the only cache invalidation this architecture has. |
| ~~Professional photography (portrait + consulting room)~~ **NOT A PENDING ITEM — 2026-08-12.** | Nothing — three real phone photographs are in use, and publication of them is authorised | — | A session is worth doing and would lift the site more than any copy change left on it. That was suggested and passed on, and it is not something this repository is waiting for. Struck rather than deleted so it is not raised as a blocker again. |
| Selection of 3–5 testimonials, and **the mechanism they come from** | Nothing — **no testimonial block was built** | Juan + Guadalupe | Held open deliberately on 2026-08-12: Instagram comments were the original plan and are no longer the only one under consideration, so the mechanism is part of the pending item and not settled. Whatever the source, the anonymisation rules in section 1 govern it. No empty block was built, because a slot invites being filled later by somebody who never read those rules. The line pointing patients at the real comments on Instagram — which is what actually provides verifiability today — is on `/contacto`. |
| ~~**"Especialista en fertilidad" on `/fertilidad`**~~ **RESOLVED 2026-08-12, hours after the climaterio wording.** | Nothing | — | Raised separately rather than changed on inference from the climaterio answer, and then answered: the `<h2>` reads "Endocrinóloga *y* **especializada** en fertilidad". Fertilidad is not a titled specialty in Argentina either — her formation is Medical Consultant in NaProTecnología — so *especialista* now appears in exactly one place on this site, `/sobre-mi`, about Endocrinología, which is the one specialty she is titled in. |
...
| ~~GA4 property created and connected inside `GTM-TCHKKB37`~~ **DONE — verified end to end 2026-08-25, and it had already been true for a fortnight.** | Nothing | — | The 2026-08-11 entry was right when written and went stale without anyone noticing. The Google tag `G-2PXQZ6PZNE` fires on *Initialization — All Pages*, `/g/collect` hits arrive, and the last 7 days held 11 users and 109 events. **It read as disconnected because the GA4 property was named `make-sense-caf66`** — renamed to `doctoracuevillas.com` on 2026-08-25 (`514219489`) and, the same day, moved out of the `Make Sense` account `375775929` into an account of its own, `doctoracuevillas.com` (`406037496`) — the state `README.md` and `measurement.md` describe. Nothing was lost; it was being recorded the whole time. The five parameters the container was dropping — `event_category`, `link_url`, `link_text`, `origen_puerta`, `origen_control` — were wired and published as container **version 3** the same day, and all ten custom dimensions are registered. *Superseded reading, 2026-08-11: the container is EMPTY — it returns its runtime and not one `G-` id.* |
| ~~Search Console claimed and sitemap submitted~~ **ALREADY DONE — established 2026-08-25 that it had been true since before 2026-08-10.** | Nothing | — | The **domain property** `sc-domain:doctoracuevillas.com` is DNS-verified by the TXT record in the DonWeb zone; Juan is verified owner and `endodracuevillas@gmail.com` has full permission. The sitemap was submitted and last read 2026-08-19, status *Correcto*, 8 pages — Google found it from the `Sitemap:` line in `robots.txt`. GA4 ↔ Search Console has been linked since 2026-08-10. `config.searchConsole.verification` correctly stays `null`: it exists for the HTML-tag route, and DNS did the verifying. **The earlier "not claimed" reading was an artefact of opening `/search-console/welcome` directly**, which always renders the add-a-property screen — see `measurement.md`, Trap 3. |
| Metricool connected to Instagram | Instagram reporting | Juan | — |
| Edit `sites.google.com/view/naprofertility` down to one line pointing here | Recovering the predecessor's traffic — **and closing a second booking funnel nobody can measure** | Juan | Cannot be done from this repository — see irreversible 4. **VERIFIED STILL OPEN 2026-09-21, and it is worse than "not done yet".** The page answers 200 with complete content: a hero on the same idea as this site's (*"Tu cuerpo no está roto"* against *"Tu cuerpo no está fallando. Está diciendo algo."*), five sections, its own navigation — Inicio, Sobre mi, Servicios, NaPro y Creighton, Contacto — and **two booking buttons pointing at `primerconsultanapro` and `seguimientonapro`, the same schedulers `config.agendas` uses.** It links to `doctoracuevillas.com` nowhere. Two consequences: the two pages compete for the same queries on a domain with Google's authority behind it, and bookings arrive through a path this site cannot observe, so `agenda_intent` is a **floor** on demand rather than a measure of it — recorded in `measurement.md` §4. No prices are shown, so that part of the original concern is closed. |
...
|---|---|---|
| ~~The five photographs in `Fotos Guada` are the practitioner, are hers to publish, and she is content to be published looking as she does in them.~~ **AUTHORISED 2026-08-12** — she sent them for this, confirmed through Juan. Three are used, cropped, at full width; one of them on two pages. **The authorisation came through Juan and not in writing from her**, which is worth knowing if it is ever questioned, and is why this row is struck rather than deleted. | — | — |
| ~~The Villanueva consulting room's address is "Complejo Vila Terra, Tigre".~~ **RESOLVED 2026-08-12 — the street line is now `Boulevard de todos los Santos 100`.** A complex is not an address: the brief named the place without saying where its door was. The centre's own site (`cmvillanueva.com.ar`) publishes "Boulevard de todos los Santos 100, Complejo Comercial Vila Terra. Tigre, Provincia de Buenos Aires" in three places on one page, and that is what `facts.js` now carries — so the footer of every page, the structured data and the map link all changed with it. The complex name and Dique Luján stayed, as orientation in the card on `/contacto`, because the place is inside a private-neighbourhood development and the street line alone will not find it. **STILL TRUE, AND THE REASON THIS IS NOT A CLOSED ITEM: nobody in this project has been to the door.** The source is the institution's own page, which is the best available and is not the same thing. | Guadalupe, or anyone who has been | A patient drives to the wrong place — now less likely than it was, and not impossible. |
| ~~`M.N. 149275` is a *matrícula nacional* issued by the Ministerio de Salud de la Nación.~~ **CONFIRMED 2026-08-12.** It is a matrícula nacional and the Ministerio de Salud de la Nación is the issuing authority. The inference held; it is now a checked fact and stands in the footer of nine pages and in the structured data. | — | — |
| ~~The three YouCanBookMe links in the brief are live and point at the right agendas. They were not opened.~~ **RETIRED 2026-08-11 — all four were opened.** Each returns HTTP 200 and names the agenda it is meant to be: `primerconsultanapro` → "PRIMER CONSULTA NAPROTECNOLOGIA", `seguimientonapro` → "SEGUIMIENTO - NAPROTECNOLOGIA", `endodracuevillas` → "ENDOCRINOLOGIA GENERAL", `climaterio` → "Consultorio CLIMATERIO / TRANSICION MENOPAUSICA". A subdomain that does not exist returns 404 at the same provider, so a 200 here is a real agenda and not a catch-all page. **This is a point-in-time check and not a monitor** — the scheduled check against the public URL is still pending, above, and a scheduler that is switched off later fails exactly as silently as one that never existed. | — | — |
...
| ~~The Austral service page URL is stable and her entry will appear there. It is used as a `sameAs` in the structured data.~~ **CONFIRMED 2026-08-12 — and her profile there is being updated.** The `sameAs` stays. | — | — |
...
| ~~The headline register changed with the design… It was shown and liked, never formally approved.~~ **APPROVED 2026-08-12.** "Tu cuerpo no está fallando. Está diciendo algo" stays, and it is now a decision rather than something nobody objected to. **Half the warmth on the page comes from this, not from the colour.** The old line survives as `facts.tagline` and in the social metadata. | — | — |
| ~~"Especialista en climaterio" is used where a patient is deciding.~~ **RESOLVED 2026-08-12 — it now reads "especializada" everywhere it referred to climaterio.** In Argentina *especialista* denotes a titled specialty with its own registration; hers is Endocrinología, and `/sobre-mi` still says *Especialista en Endocrinología* because that is exactly what it is. For climaterio she holds international training. Changed on the credential chips of the home page, `/climaterio` and `/endocrinologia`, and in two meta descriptions. **The same reasoning reaches "especialista en fertilidad" on `/fertilidad`, which was NOT part of the question and was left alone** — it is in Pending, as a decision that is hers. | — | — |
...
- **Do not link `naprofertilitydracuevillas.youcanbook.me`.** It is retired. As of 2026-09-21 it
  answers 404, so the old prices it used to show are gone with it — but the rule stands: a link from
  here re-establishes exactly the signal being removed, and now earns a broken link for it.
...
- **Do not put WhatsApp back as a primary or secondary button on a door.** It was on 24 controls
  across nine pages and is now on 12, nine of which are the single footer entry. The practice does
  ...
  and only until its agenda exists.~~ **The exception ended 2026-08-11**, when its agenda arrived:
  no door carries a WhatsApp button any more. The three outside the footer are "Otras consultas" on
  `/como-es-la-consulta` and the two deliberate controls on `/contacto`.
...
| UTM parameters on the outbound scheduler links. | Not added. | UTM parameters are read by the *destination's* analytics. YouCanBookMe does not report them back, so they would be decoration. Attribution comes from the Instagram→site UTMs, which do work, plus the `puerta` property on every event. **Held again 2026-08-11:** the climaterio agenda was supplied as the Instagram link-in-bio URL, carrying `utm_source=ig&utm_medium=social&utm_content=link_in_bio` and an `fbclid`. It is stored bare. Those parameters say how somebody reached *Instagram's* bio link, not how they reached this site — passing them on would stamp every booking from every page here as coming from the bio, which is worse than no attribution because it looks like attribution. |
...
| Cloudflare Pages, because it can serve headers and real redirects. | **Reversed to GitHub Pages.** Cloudflare required moving the DNS zone, which the client declined — and hosting is the client's call. What it costs is now declared rather than absorbed: see *Absent controls*. | The client, on reading the hosting decision. |
...
| YouCanBookMe | Agenda | `climaterio` | Climaterio / transición menopáusica. Live since 2026-08-11. The provider is configured with a 45-minute default slot; whether that is the consultation's length is Guadalupe's to confirm — see Pending | n/a (outbound link) |
...
| GitHub | Repository visibility | **public** since 2026-08-11 | Required for Pages on the organisation's free plan. The whole history is public, including every earlier version of `brief.md`; making it private again would not undo that | — |
