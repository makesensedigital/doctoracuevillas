...
| **The length of the climaterio consultation** | Nothing — the booking card and the closing copy say how the turno is arranged instead of how long it takes | Guadalupe | The other two doors state a duration; this one has never declared one in anything this repository owns. The provider is configured with a **45-minute default slot**, but that is the scheduler's setting and may carry buffer the consultation does not, so it is not copied into the page on that basis — a guessed fact reads exactly like a checked one. One sentence from her and it goes into `/climaterio`, `/como-es-la-consulta` and the booking card together. |
...
| The hero photograph for `/sobre-mi` — **a stopgap is in place since 2026-08-11** | Nothing visual. It costs a **duplicated photograph**: the page now shows the same image as `/climaterio`, framed wider | Guadalupe | The five photographs supplied on 2026-08-09 are all in use — `11.22.10` → climaterio *and ahora sobre mí*, `11.22.29` → endocrinología y la consulta, `11.22.36` → fertilidad, `11.22.45` → el retrato del home, `11.22.23` → el retrato de pasillo que estaba acá. Four are 960×1280 and publish at 760×950; **the corridor one is 640×640**, so its 4:5 crop is 512×640 and the browser was stretching it into the 760×950 box. That was the softness — not the encoding, which at 49 KB/MP sat inside the range of the others. There was no sixth photograph on disk. Sharpness was chosen over variety, deliberately and temporarily. **What ends it:** a full-size original of the corridor portrait (posed, Mater Dei lanyard — whoever took it has it) or a photograph taken for this page. Overwriting `assets/guada-sobremi.webp` then requires bumping `config.assetVersion`, because the URL does not change and `?v=` is the only cache invalidation this architecture has. |
