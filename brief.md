...
| ~~**The street number of the Escobar consulting room**~~ **CONFIRMED 2026-08-11 — closed: Juan P. Asborno 642.** | Nothing | Guadalupe | Asked directly because two numbers were in circulation and the obvious source is the one that disagrees. **The hospital's own page for this sede, `/pacientes/sedes/escobar/`, publishes 650**, and several scraped directories follow it; 642 is what Guadalupe gave, and it is also what the hospital's 2014 opening note and its Plan de Salud sucursal listing carry. She attends there. The discrepancy is written into `facts.js` beside the entry and into the card on `/contacto`, because an unexplained number that contradicts the first source anybody would check gets "fixed" by the next person to look — and a wrong number here sends a patient to the wrong door, which nothing on this site can see happen. Re-confirm if she moves consulting rooms. |
